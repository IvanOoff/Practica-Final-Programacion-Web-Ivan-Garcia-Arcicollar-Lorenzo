...
return (
      ...
      <nav className="fixed top-0 w-full bg-black text-white z-50">
        <div className="px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-3 flex-1 justify-center">    
            ...
            {/* Search Bar */}
            <div className="bg-gray-600 rounded-full px-4 py-2 flex items-center gap-2 w-80 relative">
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-gray-900 rounded-lg max-h-96 overflow-y-auto z-50">
                  {resultadosArtistas.map(artist => (
                    <div
                      ...
                      {artist.images && artist.images[0] && (<img src={artist.images[0].url} alt={artist.name} className="w-10 h-10 rounded-full" />
                      )}  
                      <div className="flex-1">
                        <p className="text-sm text-white font-semibold">{artist.name}</p>
                      </div>
                    </div>
                  ))}
                </div>
  ...
  <div className="flex-1">
    <div className="max-w-full mx-auto bg-linear-to-b from-gray-800 to-black rounded-2xl border border-gray-900 p-2">
      ...
      {playlist.length > 0 && (
        <div className="mt-8 bg-gray-900 rounded-2xl border border-gray-800 p-6">
          <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
            <h2 className="text-2xl font-bold text-white">Tu Playlist con {playlist.length} canciones</h2>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="bg-black hover:bg-gray-700 text-white font-bold py-2 px-6 rounded-lg transition">🔄
              </button>
              <button
